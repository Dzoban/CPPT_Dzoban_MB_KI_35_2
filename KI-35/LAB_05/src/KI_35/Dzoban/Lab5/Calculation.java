...
public class Calculation {
     ...
     */
    public double calculate(int x) throws CalcException, ExceptionTest {
        double y, rad;
        rad = (x * Math.PI) / 180.0;
        ...
        try {
            ...
            if (y == Double.NaN || y == Double.NEGATIVE_INFINITY || y == Double.POSITIVE_INFINITY || x == 90 || x == -90 || x==30)
                throw new ArithmeticException();
        } catch (ArithmeticException ex) {
            if (rad == Math.PI / 2.0 || rad == -Math.PI / 2.0)
                throw new CalcException("Exception reason: Illegal value of X for tangent calculation");
            else if( x==30 ){
                throw new ExceptionTest("Test new exception");
            }
            else
